...
if(isset($_GET['gender'])) {
    ...
    $query = "SELECT COUNT(*) as total FROM movie JOIN movie_genre ON movie.id = movie_genre.movie_id JOIN genre ON genre.id = movie_genre.genre_id WHERE genre.name = :genre";
    ...
    $query = "SELECT movie.id, movie.title, movie.description, 
                    ...
                    WHERE genre.name = :genre GROUP BY movie.id, movie.title
                    LIMIT :per_page OFFSET :offset";

    $datas = $db->prepare($query);
    $datas->bindValue(':genre', $genre);
        ...
        <?php if(!empty($movies)) { ?>
            ...
            <div class="search__pagination">
                ...
                <?php if ($total['total'] > 4) { ?>
                    <?php if ($page > 1) { ?>
                        <a href="gender.php?gender=<?= $genre ?>&page=<?= $page - 1 ?>" class="search__pagination-prev"><?= $page - 1 ?></a>
                    <?php } ?>
                    <span class="search__pagination-page"><?= $page ?></span>
                    <?php if (count($movies) == $per_page) { ?>
                        <a href="gender.php?gender=<?= $genre ?>&page=<?= $page + 1 ?>" class="search__pagination-next"><?= $page + 1 ?></a>
                    <?php } ?>
                <?php } ?>
            </div>
        <?php } else { ?>
